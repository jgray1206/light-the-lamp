                ...
                xhttp3.onreadystatechange = function () {
                  if (this.readyState == 4) {
                      if (this.status == 200) {
                        ...
                        var activeGame = sortedGames.reverse().find((game) => { return game.gameState == "Live" || game.gameState == "Preview" }) || sortedGames[0];
...
function createTable(game, picks, user, activeGame) {
    ...
    if (teamIsAwayOrHome == "away") {
        gameStringShort += "@" + game.homeTeam.teamName;
    } else {
       gameStringShort += "v" + game.awayTeam.teamName;
    }
