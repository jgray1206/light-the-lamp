...
open class PasswordResetController(
        ...
        return userRepository.findByEmailIgnoreCase(email)
            .flatMap { user ->
                ...
                mailService.sendEmail(
                    email, "Light the Lamp - Password Reset Request",
                    """
    ...
    open fun resetPassword(
        @QueryValue uuid: String,
        @QueryValue @NotBlank @Size(min = 8, max = 50) password: String
    ): Mono<Void> {
        return passwordResetRepository.findByResetUuid(uuid)
            .switchIfEmpty(Mono.error { error("password reset request not found, please submit another one.") })
